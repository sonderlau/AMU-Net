...
from utils.ElapsedTime import get_day_elapsed, get_year_elapsed
...
class SEVIRDataset(Dataset):
    ...
    def __init__(
        ...
        stride: int = 12,
        img_size: int = 384,
        lead_times: Tuple[int] = (10, 20, 30, 60, 120)
    ):
        """
        初始化数据集
    ...
    def _prepare_samples(self) -> List[dict]:
        ...
        min_leadtime = min(self.lead_times)
        ...
        for idx, row in filtered_catalog.iterrows():
            ...
            while start_idx + total_needed_frames < max_frames:
                ...
                for lt in self.lead_times:
                    ...
                    if start_idx + self.input_frames + lead_index < max_frames:
                        
                        utc_time = event_date + timedelta(minutes=self.input_frames * 5)
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        try:
            with h5py.File(sample_info["file_path"], "r") as f:
                ...
                data = full_event_data[:, :, sample_info["start_idx"]: sample_info["start_idx"] + self.input_frames + sample_info["lead_index"]]
                ...
                target_tensor = torch.from_numpy(data[:, :, -1:]).float().permute(2, 0, 1)
                
                # time
                
                current_time = sample_info["time"]
                ...
                return {
                    ...
                    "lead_time": sample_info["lead_time"],
                    "elapsed_time": torch.tensor([get_day_elapsed(current_time), get_year_elapsed(current_time)])
                }    
